...
const Notas = () => {
  ...
  const deleteReply = async (replyId: string) => {
    try { await adminAction("delete_reply", { reply_id: replyId }); } catch (err: any) { alert(err.message); }
  };

  const deleteQuestion = async (questionId: string) => {
    if (!confirm("¿Eliminar esta pregunta y todas sus respuestas?")) return;
    try { await adminAction("delete_question", { reply_id: questionId }); } catch (err: any) { alert(err.message); }
  };
